...
class StopServer extends Command
{
    ...
    {
        $this->setDescription('Stop zlchat servers.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$pid) {
            $io->note('zlchat server pid is invalid.');
            return -1;
        }

        if (!Process::kill($pid, SIG_DFL)) {
            $io->note('zlchat server process does not exist.');
            return -1;
        }

        if (!Process::kill($pid, SIGTERM)) {
            $io->error('zlchat server stop error.');
            return -1;
        }

        $io->success('zlchat server stop success.');
        return 0;
    }
