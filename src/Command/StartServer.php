...
class StartServer extends Command
{
    ...
    {
        $this
            ->setDescription('Start ZlChat servers.')
            ->addOption('daemonize', 'd', InputOption::VALUE_OPTIONAL, 'ZlChat server daemonize', false)
    ...
    private function startServer()
    {
        ...
        if ($this->daemonize) {
            ...
            $this->io->success('ZlChat server start success.');
        }
    ...
    private function stopServer()
    {
        ...
        if ($pid && Process::kill($pid, SIG_DFL)) {
            if (!Process::kill($pid, SIGTERM)) {
                $this->io->error('old ZlChat server stop error.');
                die();
            }
